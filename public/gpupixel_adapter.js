...
window.GPUPixelAdapter = {
    ...
    loadModule: function (displayCanvasId) {
        ...
        this.moduleLoadPromise = new Promise((resolve, reject) => {
            ...
            const testCtx = glCanvas.getContext('webgl2') || glCanvas.getContext('webgl');
            if (!testCtx) {
    ...
    startCamera: function (canvasId) {
        ...
        function checkCanvas() {
            ...
            if (!canvas) {
                attempts++;
                if (attempts % 5 === 0) {
                    console.warn(`[GPUPixel] Canvas ${canvasId} not found (${attempts}/${maxAttempts}).`);
                }
                if (attempts < maxAttempts) {
                    setTimeout(checkCanvas, 200);
                } else {
                    console.error("[GPUPixel] FATAL: Canvas " + canvasId + " not found after retries.");
                }
                return;
            }
            ...
            if (self.wasmModule && self.moduleLoadPromise) {
                ...
                self.loadModule(canvasId).then(function (wasmModule) {
        ...
        navigator.mediaDevices.getUserMedia({
            ...
            self.videoElement.onloadedmetadata = function () {
                ...
                try {
                    ...
                    const initResult = self.wasmModule.ccall('Init', 'number', ['string'], ['/gpupixel']);
                    ...
                    console.log("[GPUPixel] Default beauty applied - Smoothing:", self.defaultSmoothing, "Whitening:", self.defaultWhitening);
                    self.isActive = true;
    ...
    _startSimpleCamera: function (canvas) {
        ...
        navigator.mediaDevices.getUserMedia({
            ...
            self.videoElement.onloadedmetadata = function () {
                ...
                self.isActive = true;
                self.processLoopSimple(width, height);
                console.log("[GPUPixel] Simple camera mode started (no beauty filter)");
            };
        }).catch(function (err) {
            console.error("getUserMedia error:", err);
        });
    ...
    processLoop: function (width, height) {
        ...
        if (this.videoElement && this.videoElement.readyState >= 2) {
            ...
            try {
                ...
                } else if (this.wasmModule.wasmMemory && this.wasmModule.wasmMemory.buffer) {
        ...
        this.requestID = requestAnimationFrame(() => this.processLoop(width, height));
    },
    ...
    processLoopSimple: function (width, height) {
        ...
        this.requestID = requestAnimationFrame(() => this.processLoopSimple(width, height));
    },
